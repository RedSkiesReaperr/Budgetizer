...
<template>
  <BasicCard :loading="false">
    <v-card-title class="text-subtitle-1 text-center mb-2">{{ $t(getCategoryTranslationKey(props.category.attributes.key)) }}</v-card-title>
    <v-card-text class="pb-0 d-flex flex-row flex-wrap justify-start align-start align-content-start">
      ...
      <div class="mt-3 d-flex flex-grow-1 flex-row flex-wrap justify-center align-start align-content-stretch">
        <CategoryChip :raw-category="props.category.attributes.key" size="default"></CategoryChip>
      </div>
    </v-card-text>
  </BasicCard>
</template>
